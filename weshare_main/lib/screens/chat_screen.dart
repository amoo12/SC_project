...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamProvider.value(
        value: DatabaseServiceRides(rid: ride.rid).messages,
                          ...
                          child: IconButton(
                            ...
                            onPressed: () {
                              if (message != '') {
                                ...
                                DatabaseServiceRides(rid: ride.rid).sendMessage(
                                    ride,
                                    Message(user.uid, DateTime.now().toString(),
                                        message));
                                widget.theMessage.clear();
                                message = '';
                                sent = false;
  ...
  Container addAvatar(String uid) {
    if (uid != user.uid) {
      return Container(
          child: CircleAvatar(
        child: FutureBuilder(
            future: DatabaseServiceUser().getUserDetails(message.uid),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                if (snapshot.data.photo) {
                  FutureBuilder(
                    future: DatabaseServiceUser().getImage(message.uid),
                    builder: (context, snapshot) {
